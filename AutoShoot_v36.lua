...
local AutoShootDerivMult   = 4.5    -- studs деривации при d=100.
local AutoShootGravScale   = 1.0    -- множитель гравитационной компенсации (1.0 = стандарт).
                                    -- Мяч летит выше → уменьши. Не долетает по высоте → увеличь. Мяч улетает меньше → увеличь.
...
local function DrawTrajectory(startPos, peakPos, endPos, spinStr)
    ...
    local P0 = startPos
    -- P1 ближе к пику (высокий вес 0.78) → спин и дуга видны с первых метров
    local P1 = Vector3.new(startPos.X + (peakLateral.X-startPos.X)*0.78,
                           startPos.Y + (peakLateral.Y-startPos.Y)*0.92,
                           startPos.Z + (peakLateral.Z-startPos.Z)*0.78)
...
local function CalcLaunchDir(startPos, targetPos)
    ...
    local gravComp = 0.5 * GRAVITY * t * t * AutoShootGravScale
...
local function GetTarget(dist, gkX, gkY, isAggressive, gkHrp, gkVel)
    ...
    for _, xf in ipairs(xPoints) do
        for _, yf in ipairs(yFracs) do
            ...
            -- idealPos: точка на плоскости ворот (для отображения и скоринга)
            ...
            -- Навес/свечка: ценен когда GK низко или выходит вперёд
            if isLobShot then
                ...
                -- Доп. бонус за "свечку" — такой удар труднее парировать
                if dist > 60 then score = score + 3.5 end
            end

            -- Верхний угол из любой дистанции — самый сложный для вратаря
            if isTopCorner and dist > 55 then score = score + 3.0 end
            ...
            -- Глубина цели: при dist < SPIN_TRICK_DIST сдвигаем цель вглубь ворот (Z-),
            -- чтобы сервер "видел" нужную дистанцию и применял спин/навес.
            -- Математически: server_dist = sqrt(dist² + zDeep²) >= SPIN_TRICK_DIST+8
            local needServerDist = SPIN_TRICK_DIST + 8
            local zDeepRaw = needServerDist*needServerDist - dist*dist
            local zDeep = (dist < SPIN_TRICK_DIST and (spinDir ~= "None" or isLobShot))
                          and (zDeepRaw > 0 and math.sqrt(zDeepRaw) or 0)
                          or  0
            -- Для всех дистанций добавляем небольшую глубину (более крутой вход в ворота)
            local zBase = 4.0
            local zFinal = zBase + zDeep  -- отрицательный Z = вглубь ворот
            local shootPos = GoalCFrame * Vector3.new(shootLocalX, localY, -zFinal)
            ...
            if vyAtGoal > 0 then
                ...
                if aimLocalY > safeTop then
                    ...
                    score = score - over * over * 25.0 - over * 8.0
                end
            end
        ...
        for _, pd in ipairs(postDefs) do
            ...
            for _, hf in ipairs({0.20, 0.45, 0.72}) do
                ...
                if goesIn then
                    ...
                    -- Рикошет = сложная атака (труднее вратарю)
                    rscore = rscore + 4.0   -- "сложность" бонус vs прямых ударов
...
local function CalculateTarget()
    ...
    -- Z-offset уже обеспечивает нужную дистанцию на сервере (см. GetTarget).
    -- ShootVel = стандартный (FIXED_POWER * 1400).
...
local function SetupUI(UI)
    if UI.Sections.AutoShoot then
        ...
        uiElements.AutoShootDragComp = UI.Sections.AutoShoot:Slider({
            Name = "Drag k (1/s)", Minimum = 0, Maximum = 3.0,
            Default = AutoShootDragComp, Precision = 2,
            Callback = function(v) AutoShootDragComp = v end
        }, "AutoShootDragComp")
        UI.Sections.AutoShoot:SubLabel({Text = "k: дальние не долетают → увеличь | близкие высоко → уменьшай"})

        uiElements.AutoShootDerivMult = UI.Sections.AutoShoot:Slider({
            Name = "Spin Drift", Minimum = 0.5, Maximum = 15,
            Default = AutoShootDerivMult, Precision = 1,
            Callback = function(v) AutoShootDerivMult = v end
        }, "AutoShootDerivMult")
        UI.Sections.AutoShoot:SubLabel({Text = "Спин уводит не так сильно → увеличь"})

        uiElements.AutoShootGravScale = UI.Sections.AutoShoot:Slider({
            Name = "Grav Scale", Minimum = 0.75, Maximum = 1.25,
            Default = AutoShootGravScale, Precision = 2,
            Callback = function(v) AutoShootGravScale = v end
        }, "AutoShootGravScale")
        UI.Sections.AutoShoot:SubLabel({Text = "Ближние высоко → уменьши | Дальние низко → увеличь"})
