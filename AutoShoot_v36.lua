...
local function DrawTrajectory(startPos, peakPos, endPos, spinStr)
    ...
    -- Перпендикуляр к траектории (горизонталь) для Magnus-смещения
    ...
    -- f_peak: при каком t параметре происходит пик дуги.
    -- Определяем по горизонтальному расстоянию до peakPos относительно endPos.
    -- Это исправляет проблему "дуга в конце": пик теперь в правильном месте.
    local totalH = (Vector3.new(endPos.X-startPos.X, 0, endPos.Z-startPos.Z)).Magnitude
    local peakH  = (Vector3.new(peakPos.X-startPos.X, 0, peakPos.Z-startPos.Z)).Magnitude
    local f = (totalH > 0.1) and math.clamp(peakH / totalH, 0.15, 0.80) or 0.40

    -- Управляющая точка квадратичного Безье для прохождения через peakPos при t=f:
    --   B(f) = (1-f)^2*P0 + 2f(1-f)*Pc + f^2*P1 = peakPos
    --   Pc = (peakPos - (1-f)^2*start - f^2*end) / (2f(1-f))
    local Pc = (peakPos - startPos*((1-f)*(1-f)) - endPos*(f*f)) / (2*f*(1-f))

    -- Magnus: "Right"→мяч влево (знак -1), "Left"→мяч вправо (+1) — инверсия в игре.
    -- Кривизна sin(π·t): 0 в начале, max в середине, 0 в конце.
    -- Это отображает реальную дугу: мяч стартует прямо, изгибается в середине,
    -- возвращается к цели — именно так работает Магнус при нашей компенсации деривацией.
    local spinSign = (spinStr == "Right") and -1 or (spinStr == "Left") and 1 or 0
    local maxCurl  = spinSign * 2.2

    local function point(t)
        local mt  = 1 - t
        local arc = startPos*(mt*mt) + Pc*(2*mt*t) + endPos*(t*t)
        -- sin(π·t): точно 0 на старте и финише, пик ровно в середине
        local curl = right * maxCurl * math.sin(math.pi * t)
        return arc + curl
    end
...
local function GetTarget(dist, gkX, gkY, isAggressive, gkHrp, gkVel)
    ...
    for _, xf in ipairs(xPoints) do
        for _, yf in ipairs(yFracs) do
            ...
            -- Дальние: поощряем высокие цели (мяч уже падает вниз, GK сложнее)
            if dist > 100 then
                score = score + (localY / math.max(yRange, 1)) * 3.5
            end
            -- Ближние: штрафуем высокие цели (риск перекладины)
            if dist < 55 then
                local hf = localY / math.max(yRange, 1)
                score = score - hf * hf * 4.5
            end
        ...
        for _, pd in ipairs(postDefs) do
            ...
            for _, hf in ipairs({0.20, 0.45, 0.72}) do
                ...
                if goesIn then
                    ...
                    rscore = rscore + 6.0  -- рикошет: приоритет сложной атаки
