...
local GOAL_DEPTH_MAX         = 3.4                  -- максимально целимся вглубь на дальних
...
local function CalcLaunchDir(startPos, targetPos)
    ...
    -- gravRamp: плавное нарастание компенсации с дистанцией.
    -- aeroCoeff растёт по времени полёта: на средних дистанциях ниже прежнего 0.21,
    -- на очень дальних чуть выше, чтобы 200–220 studs долетали без перезавышения ближних ударов.
    local gravRamp = 1 - math.exp(-(t / 0.26) ^ 2)
    local gravComp = 0.5 * GRAVITY * t * t * gravRamp
    local aeroCoeff = 0.15 * (1 + 0.75 * t)
    local aeroLoss = aeroCoeff * k * V * t * t * gravRamp
...
local function GetTarget(dist, gkX, gkY, isAggressive, gkHrp, gkVel)
    ...
    for _, xf in ipairs(xPoints) do
        for _, yf in ipairs(yFracs) do
            ...
            score = score + hFrac * math.clamp((dist - 95) / 55, 0, 1) * 2.4
