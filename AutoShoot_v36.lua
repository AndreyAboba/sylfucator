...
-- DrawTrajectory: параметрическая формула — дуга и Магнус отображаются правильно.
-- Дуга:  4t(1-t) → пик строго в t=0.5 (середина пути).
-- Спин:  4t(1-t) → заметен сразу от t=0+, пик в t=0.5, нуль при приземлении.
-- Это физически корректно: Магнус действует с самого начала полёта.
local function DrawTrajectory(startPos, peakPos, endPos, spinStr)
    ...
    local driftAmt = spinSign * 1.8  -- боковое смещение на пике (studs)

    -- Высота пика относительно прямой линии start→end
    local midY = (startPos.Y + endPos.Y) * 0.5
    local peakExcess = peakPos.Y - midY

    local function trajPos(t)
        -- Прямолинейная интерполяция
        local p = startPos + (endPos - startPos) * t
        -- Вертикальная дуга: симметричный параболический горб, пик в t=0.5
        p = Vector3.new(p.X, p.Y + peakExcess * (4*t*(1-t)), p.Z)
        -- Боковой Магнус: 4t(1-t) — заметен с t=0+, нуль на старте и при приземлении
        p = p + right * (driftAmt * 4*t*(1-t))
        return p
    end
...
local function GetTarget(dist, gkX, gkY, isAggressive, gkHrp, gkVel)
    ...
    for _, xf in ipairs(xPoints) do
        for _, yf in ipairs(yFracs) do
            local localX = xf
            -- Y_TOP_INSET адаптируется к дистанции:
            --   Близко (мяч поднимается при пересечении) → полный инсет, не задеть перекладину.
            --   Далеко (мяч падает при пересечении)      → минимальный инсет, цель может быть выше.
            -- Плавный перегрев: от Y_TOP_INSET (d=0) до 0.15 (d≥160 studs).
            local distTopInset = math.max(0.15, Y_TOP_INSET * math.max(0, 1 - approxT / 0.40))
            local yRange = math.max(0.5, GoalHeight - distTopInset - Y_BOT_INSET)
            ...
            -- Навес (свечка): мяч падает сверху → GK прыгает поздно, труднее отбить
            if isLobShot then
                score = score + 2.5  -- базовый бонус за трудность
            ...
            if vyAtGoal > 0 then
                ...
                local safeTop      = GoalHeight - distTopInset
        ...
        for _, pd in ipairs(postDefs) do
            ...
            for _, hf in ipairs({0.20, 0.45, 0.72}) do
                ...
                if goesIn then
                    ...
                    -- Рикошет = внезапный, GK реагирует медленнее → бонус за сложность
                    rscore = rscore + 4.0
...
local function CalculateTarget()
    ...
    -- TRICK: при dist < SPIN_TRICK_DIST сервер не применяет закручивание/навесы.
    -- Метод: целимся ЗА ворота — сервер видит dist = CurrentDist+extra > SPIN_TRICK_DIST.
    -- Мяч всё равно попадает в ворота (проходит через плоскость ворот по пути к дальней точке).
    local needsTrick = CurrentDist < SPIN_TRICK_DIST and (CurrentSpin ~= "None" or CurrentIsLob)
    if needsTrick then
        local extra     = math.max(12, SPIN_TRICK_DIST - CurrentDist + 15)
        local flatDir   = Vector3.new(CurrentLaunchDir.X, 0, CurrentLaunchDir.Z).Unit
        local landY     = PredictedLand and PredictedLand.Y or (GoalFloorY or 0) + 2
        local ballS     = GetBallStartPos()
        local farTarget = Vector3.new(ballS.X + flatDir.X*(CurrentDist + extra),
                                      landY,
                                      ballS.Z + flatDir.Z*(CurrentDist + extra))
        local extDir, _, _ = CalcLaunchDir(ballS, farTarget)
        ShootVel = extDir * (FIXED_POWER * 1400)
    else
