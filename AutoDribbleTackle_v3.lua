...
local AutoDribbleConfig = {
    ...
    TacklerMinSpeed = 8,          -- Минимальная скорость такля чтобы считать его активным
}
...
local function ShouldDribbleNow(specificTarget, tacklerData)
    ...
    -- Записываем историю позиций такла
    RecordDribbleTargetPos(specificTarget, tacklerRoot)
    local tacklerRealVel = GetDribbleTargetVelocity(specificTarget)
    local tacklerSpeed = Vector3.new(tacklerRealVel.X, 0, tacklerRealVel.Z).Magnitude

    -- Если таклер почти стоит (скорость меньше минимума) — не реагируем
    if tacklerSpeed < AutoDribbleConfig.TacklerMinSpeed then
        if Gui and AutoDribbleConfig.Enabled then
            Gui.AngleLabel.Text = string.format("Speed: %.1f (slow)", tacklerSpeed)
        end
        return false
    end

    local myPos = HumanoidRootPart.Position
    local tacklerPos = tacklerRoot.Position
    local toMe = (Vector3.new(myPos.X, 0, myPos.Z) - Vector3.new(tacklerPos.X, 0, tacklerPos.Z))
    local distFlat = toMe.Magnitude

    if distFlat < 0.1 then return false end
    if distFlat > AutoDribbleConfig.MaxDribbleDistance then return false end

    -- Направление движения таклера (position-based, устойчиво к fakes)
    local tacklerDir = Vector3.new(tacklerRealVel.X, 0, tacklerRealVel.Z).Unit
    local dirToMe = toMe.Unit

    -- Угол между вектором движения таклера и направлением к нам
    ...
    -- Таклер смотрит достаточно в нашу сторону
    if angle > AutoDribbleConfig.MinAngleForDribble then return false end

    -- Время до столкновения
    local relativeSpeed = tacklerSpeed + math.max(HumanoidRootPart.AssemblyLinearVelocity.Magnitude, 1)
    local timeToCollision = distFlat / relativeSpeed

    -- При маленьком расстоянии — реагируем сразу
    if distFlat <= AutoDribbleConfig.DribbleActivationDistance then
        AutoDribbleStatus.TackleDetectionCooldown = currentTime
        return true
    end

    -- Чуть дальше — только если летит быстро и близко столкнётся
...
local function SetupUI(UI)
    ...
    if UI.Sections.AutoDribble then
        ...
        uiElements.AutoDribbleTacklerMinSpeed = UI.Sections.AutoDribble:Slider({
            Name = "Tackler Min Speed", Minimum = 1, Maximum = 30,
            Default = AutoDribbleConfig.TacklerMinSpeed, Precision = 1,
            Callback = function(v) AutoDribbleConfig.TacklerMinSpeed = v end
        }, "AutoDribbleTacklerMinSpeed")
...
local function SynchronizeConfigValues()
    ...
    local pairs_sync = {
        ...
        { uiElements.AutoDribbleTacklerMinSpeed,  function(v) AutoDribbleConfig.TacklerMinSpeed = v end },
    }
